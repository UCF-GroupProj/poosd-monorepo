...
export class PWDReset extends RouteHandle {
  ...
  async postHandle(req: Request<unknown, unknown, { email: string }>, res: Response) {
    ...
    const record = await ReqColl.findOneAndUpdate(
      { userId: userPro._id, requestType: "password" },
      {
        $set: {
          requestId: reqID,
          createdAt: new Date(),
  ...
  async patchHandle(
    req: Request<{ ResetCode: string; }, unknown, { newPassword: string }>,
    res: Response
  ) {
    // 🔧 FIX: proper empty-check and status()
    if (!req.body.newPassword || req.body.newPassword.trim() === "")
      return res.status(400).send("Missing newPassword Field");
    ...
    try {
      await DBSession.withTransaction(async () => {
        ...
        if (!codeData)
          throw new pwdPatchErr(
            logger.fmt`Attempt to reset user password but invalid code provided: ${req.params.ResetCode}`,
            PatchErrCode.badResetCode
          );

        // Update password
        const pwdUpdateState = await userColl.updateOne(
          { _id: codeData.userId },
          {
            $set: {
        ...
        if (!pwdUpdateState.acknowledged)
          throw new pwdPatchErr(
            logger.fmt`Database failed to respond to a password update request`,
            PatchErrCode.noDBResponse
          );
        if (pwdUpdateState.modifiedCount === 0)
          throw new pwdPatchErr(
            logger.fmt`No user account had their password updated: ${codeData.userId.toHexString()}`,
            PatchErrCode.genericErr
          );
        ...
        if (!rmPwdReqState.acknowledged)
          throw new pwdPatchErr(
            logger.fmt`Database failed to respond to a user request deletion request`,
            PatchErrCode.noDBResponse
          );
        if (rmPwdReqState.deletedCount > 1)
          logger.warn(
            logger.fmt`User ${codeData.userId.toHexString()} found multiple password reset request in the database`
          );
        if (rmPwdReqState.deletedCount === 0)
          throw new pwdPatchErr(
            logger.fmt`Request code existed before the deletion were called?? Did DBA tempered with records??: ${codeData.userId.toHexString()}`,
            PatchErrCode.genericErr
          );
